...
#Info: Pause the printer at a certain height to change filament
#Depend: GCode
#Type: postprocess
#Param: pauseLevel(float:5.0) Change height (mm)

__copyright__ = "Copyright (C) 2017 Bas Delfos - Released under terms of the AGPLv3 License"
import re
	...
	for lIndex in xrange(len(lines)):
		...
		if pauseState == 2:
			...
			if g == 1 or g == 0:# We will do the pause just before printing content. We need to pause from the previous XY position. Not the current.
				...
				pauseState = 0
                #Change filament
				f.write("\nM600 E-5.0 X0.0 Y0.0 Z5   ; change filament\n\n")
